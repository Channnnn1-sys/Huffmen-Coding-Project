...
#* Analysis constants for entropy detection and safe file processing
ZIP_BASED_OFFICE_EXTENSIONS = {"docx", "pptx", "xlsx"}
COMPRESSED_SUFFIX = "-compressed.bin"
# Sample size for lightweight entropy heuristics (kept small)
...
=======
def build_response_archive_bytes(content_bytes, inner_filename, archive_name, metadata_file_paths, target_dir):
    """#* Create a ZIP archive with content and C++ generated metadata."""
...
def load_json_file(path):
    """#* Safely load a JSON file generated by the native C++ engine."""
    try:
...
def build_response_archive_bytes(content_bytes, inner_filename, archive_name, metadata_entries, target_dir):
    """#* Create a ZIP archive with content and native metadata files."""
    archive_path = target_dir / archive_name
    with zipfile.ZipFile(archive_path, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr(inner_filename, content_bytes)
        for metadata_path, archive_name in metadata_entries:
            if metadata_path.exists():
                archive.write(metadata_path, archive_name)
>>>>>>> 43f042d (C++ codes increased!)
    return base64.b64encode(archive_path.read_bytes()).decode()
...
=======
            ...
            # Prefer the actual decompressed file (non-JSON) if multiple matching files exist
            candidates = [c for c in output_dir.iterdir() if c.is_file() and c.name.startswith(f"{base_name}-decompressed")]
            for candidate in candidates:
                if not candidate.name.lower().endswith('.json'):
                    expected_output = candidate
                    break
            # Fallback to any candidate (including JSON) if no non-JSON found
            if expected_output is None and candidates:
...
=======
                ...
                # C++ engine should generate metadata alongside the output binary
                metadata_path = output_file.parent / f"{output_file.stem}-metadata.json"
                report_path = output_file.parent / f"{output_file.stem}-compression_report.json"
...
=======
                    "original_size": report.get("original_size", 0),
                    "compressed_size": report.get("compressed_size", 0),
                    "compression_ratio": report.get("compression_ratio", 0.0),
                    "saved_bytes": report.get("saved_bytes", 0),
                    "entropy": report.get("entropy", 0.0),
                    "entropy_warning": report.get("entropy_warning"),
                    "average_bits_per_symbol": report.get("average_bits_per_symbol", 0.0),
                    "efficiency_vs_8bit": report.get("efficiency_vs_8bit", 0.0),
                    "top_symbols": report.get("top_symbols", []),
                    "file_type": report.get("file_extension", get_extension(filename)),
>>>>>>> 43f042d (C++ codes increased!)
...
=======
                ...
                report_path = output_file.parent / f"{output_file.stem}-decompression_report.json"
                processing_time = (datetime.now() - start_time).total_seconds()
                ...
                data_b64 = build_response_archive_bytes(
                    ...
                    zip_name,
                    [
                        (output_file.parent / f"{output_file.stem}-metadata.json", "metadata.json"),
                        (report_path, "decompression_report.json")
                    ],
                    temp_session_path
>>>>>>> 43f042d (C++ codes increased!)
                )
